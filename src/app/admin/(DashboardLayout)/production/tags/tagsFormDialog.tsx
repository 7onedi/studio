...
interface Props {
  open: boolean;
  initial?: Partial<Tags>;
  onClose: () => void;
  onSaved: (cat: Tags) => void;
}

export default function TagsFormDialog({ open, initial, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const url = isEdit ? `/api/tags/${initial!.id}` : '/api/tags';
